...
_logger = logging.getLogger("lirox.pipeline.ux")
...
class UXStrategist:
    ...
    def _design_document(self, intent) -> SiteStructure:
        """LLM-driven structure generation."""
        prompt = f"""\
...
"""
        try:
            ...
            sections = [
                Section(name=s["name"], purpose=s["purpose"], content_type="text", cta="")
                for s in items if isinstance(s, dict) and "name" in s
            ]
            if sections:
                return SiteStructure(
                    sections=sections,
