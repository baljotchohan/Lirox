...
from lirox.agents.classifier import _classify, SELF_SIGNALS, MEMORY_SIGNALS
...
def _get_sys(profile_data: Dict[str, Any]) -> str:
    ...
    base += (
        ...
        "\n"
        "GREETING RULE — ABSOLUTE (zero exceptions):\n"
        "• When user says hi/hello/hey/hlo/sup/yo/howdy or any casual greeting:\n"
        "  → Reply in 1-2 sentences MAX. Warm, casual, conversational.\n"
        "  → NEVER list your capabilities in a greeting response.\n"
        "  → NEVER output a '📝 Introduction' or capabilities block.\n"
        "  → NEVER use headers, bullets, or sections in a greeting.\n"
        "  → CORRECT: 'Hey! What are we building today?'\n"
        "  → WRONG: Any response mentioning filesystem access, capabilities, or tools.\n"
        "  → The greeting MUST feel human. One line is ideal.\n"
        "\n"
        "MEMORY & LEARNING:\n"
...
# Removed classifier logic to lirox/agents/classifier.py to resolve circular imports
